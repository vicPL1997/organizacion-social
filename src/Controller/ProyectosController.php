...
class ProyectosController extends AbstractController
{
    ...
    public function verProyecto(Request $request,  Doctrine\Persistence\ManagerRegistry $doctrine, UserPasswordHasherInterface $passwordHasher, SluggerInterface $slugger, ChartBuilderInterface $chartBuilder,$id, ProyectosRepository $proyectos, UserRepository $users, $tab=1 ){

        $proyecto = $proyectos ->find($id);
        $usuarios=$users->verUsuariosDelProyecto($id);
        $usuariosBeneficiarios=$users->findBy(array('rol' => 'Usuario',
            'comunidadAutonoma' => $proyecto->getSede()->getLocalizacion()));

        $adminSede=$proyecto->getSede()->getAdministradorSede()->getId();


        $voluntarios=$users->verVoluntariosDelProyecto($id);
        $usuariosVoluntarios=$users->findBy(array('rol' => 'Voluntario',
            'comunidadAutonoma' => $proyecto->getSede()->getLocalizacion()));
        ...
        if($form->isSubmitted() && $form->isValid()){
            ...
            if ($imgFile) {
                ...
                unlink($archivos);
                $this->addFlash('exitoUserExcel','Se han registrado los usuarios exitosamente y se han añadido correctamente a el proyecto');
                return $this->redirectToRoute('verProyecto', ['id'=>$id, 'tab'=>2]);
            ...
            'empleados' => $empleados, 'usuariosEmpleados' => $usuariosEmpleados,'formulario' => $form->createView()
    ...
    private function estadisticasEstadoCivil(ChartBuilderInterface $chartBuilder, $usuarios){
        $chart = $chartBuilder->createChart(Chart::TYPE_PIE);
        $chart->setData([
            'labels' => ['Solteros ('.self::obtenerPorcentajeEstadoCivil($usuarios, "Soltero").'%)',
                'Casados('.self::obtenerPorcentajeEstadoCivil($usuarios, "Casado").'%)'],
            'datasets' => [
                [
                    'label' => '% de géneros',
